...
class Area(object):
    ...
    def count_stay(self):
        tmp_frame = self.work_frame[(self.work_frame['frame_number'] > self.last_frame_num)]
...
def main(work_frame, fps, length_of_video=4500, location='1', start_time=0, end_time=0):
    # 30fps. 9000
    ...
    last_frame_num = int((end_time_datetime - start_time_datetime).total_seconds()) * fps
    ...
    if start_time == 0:
        ...
        total_frame.to_csv('count_results/location-' + location + '-' + start_time + '.csv', index=False)
    print('all good')

...
class Area(object):
    ...
    def count_stay(self):
        tmp_frame = self.work_frame[
            (self.work_frame['frame_number'] == self.last_frame_num)
            | (self.work_frame['frame_number'] == self.last_frame_num - 1)
            | (self.work_frame['frame_number'] == self.last_frame_num - 2)
            ]
...
def main(work_frame, fps, length_of_video=4500, location='1', start_time=0, end_time=0, last_frame_num=9000):
    ...
    if start_time == 0:
        ...
        count_results_path = ('count_results\location-' + location + '-' + start_time + '.csv').replace(' ',
                                                                                                        '-').replace(
            ':', '-')
